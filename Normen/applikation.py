...
def create_pdf(dataframe):
    ...
    try:
        ...
        if isinstance(pdf_output, str):
            ...
            st.error("Unerwartetes Ausgabeformat der PDF")
            return None
    except Exception as e:
    ...
    if len(gefiltert) > 0 and PDF_AVAILABLE:
        with st.spinner('Erstelle PDF...'):
            try:
                ...
                if pdf_data is None:
                    st.warning("PDF konnte nicht erstellt werden. Versuche stattdessen CSV-Export...")
                    raise Exception("PDF-Daten sind None")
                ...
                if not isinstance(pdf_data, bytes):
                    raise ValueError("Ungültiges PDF-Datenformat")
                ...
                try:
                    ...
                    st.error(f"Fehler beim Erstellen der CSV: {str(csv_error)}")
    elif len(gefiltert) > 0:
        # Show CSV download as fallback if PDF is not available
        try:
            csv = gefiltert.to_csv(index=False).encode('utf-8')
            b64 = base64.b64encode(csv).decode('utf-8')
            href = f'<a href="data:file/csv;base64,{b64}" download="normen_uebersicht.csv">Ergebnis als CSV herunterladen</a>'
